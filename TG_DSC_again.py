...
def compute(T_1,CR,pos):
    ...
    A=Symbol('A')
    aa=math.exp(B1)*E*Speed_seed[pos%3]/8.314
...
def do_it():
    ...
    for i in range(len(seed)):
        ...
        for j in range(100,50000):
            if (x1[j]<280)&(x1[j+5]>280):
                T_l1=j
                break
        for j in range(T_l1,50000):
            if (x1[j]<390)&(x1[j+5]>390):
                T_l2=j
                break
        l1,l2,rr,A1,B1,E,A=changeline(x1,y1,z1,T_l1-5,T_l1+5,T_l2-5,T_l2+5,seed[i])
        name.append(file)
        num1=x1[l1]
        num2=x1[l2]
        temp_str=str(num1)+'->'+str(num2)
        Range.append(temp_str)
        num1=(y1[30]-y1[l1])/(y1[30]-y1[y1.size-100])
        num2=(y1[30]-y1[l2])/(y1[30]-y1[y1.size-100])
        temp_str=str(num1)+'->'+str(num2)
